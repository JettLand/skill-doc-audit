...
- `structure`：结构体检 + 元信息。**advisory 分层（v1.38.0）**：`version_missing`/`name_missing`/`desc_missing`/`broken_ref` 属真实结构缺陷（非 advisory，计入「结构真实缺陷」信号）；`name_mismatch`/`license_missing`/`desc_length`/`desc_four`/`h1_name_mismatch`/`no_frontmatter`/`too_long`/`hardcoded_path`/`todo_marker`/`placeholder` 属风格建议（标记 `advisory=True`，单独统计、不参与「真实缺陷」分母），以消解 structure 在杂乱语料上 100% 命中导致的信号饱和。
- `security`：安全红线静态子集
- `runtime`：脚本可运行性
- `deps`：依赖与平台声明
...
| doc-llm | `doc_llm_agent_handoff` | （v1.37.0 起不再作为 finding 产出）语义漂移检测交接信号改由 stderr `AGENT_TAKEOVER` 哨兵 + 回执 `_meta.handoff_dossier` 传达；本标签仅保留作历史兼容 | 用户选「agent 接手」或显式 `--doc-llm-mode agent`：脚本写 dossier（SKILL.md 全文 + 代码事实清单 + 正向覆盖缺口预分析 + 比对要点）并打 `AGENT_TAKEOVER` 哨兵，agent 读取后自行比对；交接不再计入 findings，避免 doc-llm 在 agent 模式下恒有 1 条 INFO 的假象 | — |
