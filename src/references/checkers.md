...
| doc | `DOC_LLM_DRIFT` | 文档/代码语义漂移（LLM 判定） | `doc-llm` 检查器（v1.23.0 起纳入 `--all-checks`）经 OpenAI 兼容 LLM 判定的语义漂移条目（格式 `- 文件:行 | 描述`），仅作线索（Vector 2，v1.22.1） | WARN |
| doc | `doc_llm_unavailable` | LLM 语义检测不可用（已跳过） | 用户**显式**用 `auto`/`ask` 要求语义检测却未能运行（未配置 LLM，或非交互环境无法询问——`ask` 呈现实选项：默认/增强/预览代价，30 秒超时回退默认），显著告警（对应 deadcode 的 precision_degraded）（Vector 2，v1.22.1） | WARN |
| doc | `doc_llm_skipped` | 全量检测中 LLM 语义检测跳过 | `--all-checks` 全量自带 `doc-llm`、非交互环境无法向用户询问 → 跳过且未调用 LLM，属预期行为，记 INFO 不告警（保「全量检测 WARN 0」不变量）（Vector 2，v1.23.0） | INFO |
| doc | `doc_llm_ran` | LLM 语义检测已运行（无漂移） | `doc-llm` 已运行且未检出语义漂移的确认性 INFO（Vector 2，v1.22.1） | INFO |
