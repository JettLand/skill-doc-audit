...
| doc | `DOC_LLM_DRIFT` | 文档/代码语义漂移（LLM 判定） | 选装 `doc-llm` 检查器经 OpenAI 兼容 LLM 判定的语义漂移条目（格式 `- 文件:行 | 描述`），仅作线索（Vector 2，v1.22.1） | WARN |
| doc | `doc_llm_unavailable` | LLM 语义检测不可用（已跳过） | 用户要求 `auto`/`ask` 却未配置 LLM，或非交互环境无法询问（`ask` 呈现实选项：默认/增强/预览代价，30 秒超时回退默认），语义检测被跳过——显著告警（对应 deadcode 的 precision_degraded）（Vector 2，v1.22.1） | WARN |
