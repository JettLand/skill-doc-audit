...
| `--deadcode-mode {ask,vulture,ast,skip}` | deadcode 精度模式；`ask` 默认已装 vulture 则自动高精度(不询问)，未装则交互询问、30s 超时或非交互回退 `ast`（**不安装**）；显式 `vulture` 或交互选 vulture 缺库时先自动安装、失败回退 `ast` 并 WARN 告警；`ast`/`skip` 绝不联网安装 |
