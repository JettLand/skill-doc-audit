...
def _write_doc_llm_dossier(ctx):
    ...
    content = (
        ...
        "## 比对要点\n"
        "先完成下方「一致性」核对，再完成「内容完整性 / 质量门槛」核对。两类均属语义判读，"
        "请报告确有依据的缺口（文档缺关键信息 / 说法与代码冲突），不要给分项评分、不要报风格问题。\n\n"
        "### 一致性（Vector 2：文档 ↔ 代码）\n"
        "1. 先核对上方「正向覆盖缺口」：这些项代码已具备但文档未写，是最可能漏更新之处，"
        "优先补文档或确认是否应移除代码能力。\n"
        "2. 逐条核对各文档声称的：能力范围、默认值、行为、数量、集合、CLI 参数、退出码、配置项 —— "
        "是否与代码事实清单一致。\n\n"
        "### 内容完整性 / 质量门槛（来自 TRACE 评测体系的互补维度，本检查器原不覆盖）\n"
        "3. 异常处理（R1）：是否写明异常发生的 文件:行:问题:修法，而非仅一句「可能报错」。\n"
        "4. 功能完整性 / 运行稳定性（R2/R3）：主要使用场景是否覆盖、有无输出格式说明 / 兜底 / 备份说明。\n"
        "5. 触发方式（A2）：是否给出正例 + 反例对照，说明何时该用、何时不该用。\n"
        "6. 文档质量（C2）：命令与错误说明是否清楚、是否提供完整可复制的输出示例。\n"
        "7. 渐进式披露（C3）：是否有「快速开始 → 进阶 → 深度参考」的分层结构。\n"
        "8. 输出准确性（E1）：每条问题是否带中文标签 + 修复建议，避免只罗列现象。\n"
        "9. 开箱即用度（E4）：常用场景是否能一条命令 / 一键完成，而非多步手工拼装。\n"
        "10. 国内适配性（T1，可选）：是否贴合国内用户习惯、是否强依赖境外网络 / 服务（仅供上架前自评参考，不计入缺陷）。\n"
