...
# examples-skill：泛用版 examples 检查器夹具（v1.26.0）。
# 覆盖四类静态场景：① 引用不存在的脚本(EXAMPLE_TARGET_MISSING ERROR) ② 危险命令(sudo rm -rf, EXAMPLE_DANGEROUS ERROR)
# ③ 调用未声明外部 CLI(curl, EXAMPLE_EXT_CMD INFO) ④ 带 expected 标注但未执行(static 模式 EXAMPLE_UNVERIFIED INFO)。
# demo.py 用 argparse 显式声明 --dry-run/--check，验证「脚本存在且参数已声明」的健康示例不误报。
FIXTURES["examples-skill"] = {"files": {
    "SKILL.md": "---\nname: examples-skill\ndisplayName: Examples Skill\ndescription: 用于验证泛用版 examples 检查器的夹具，覆盖示例引用缺失/危险命令/未声明外部CLI/带期望标注四类场景。\nversion: \"0.1.0\"\nlicense: MIT\nauthor: test\ntags: [test]\n---\n\n# Examples Skill\n\n## 用法\n\n正常示例（脚本存在、参数已声明）：\n\n```bash\npython scripts/demo.py --dry-run\n```\n\n引用不存在的文件（应报 EXAMPLE_TARGET_MISSING）：\n\n```bash\npython scripts/missing_tool.py --x\n```\n\n危险命令（应报 EXAMPLE_DANGEROUS）：\n\n```bash\nsudo rm -rf /tmp/scratch\n```\n\n调用未声明外部命令（应报 EXAMPLE_EXT_CMD）：\n\n```bash\ncurl https://example.com/api\n```\n\n带期望标注但未执行（static 模式应报 EXAMPLE_UNVERIFIED）：\n\n```bash {example expected-exit=0 expected-stdout=\"done\"}\npython scripts/demo.py --check\n```\n",
    "scripts/demo.py": "import argparse\n\n\ndef main():\n    p = argparse.ArgumentParser()\n    p.add_argument(\"--dry-run\", action=\"store_true\")\n    p.add_argument(\"--check\", action=\"store_true\")\n    args = p.parse_args()\n    if args.check:\n        print(\"done\")\n    return 0\n\n\nif __name__ == \"__main__\":\n    main()\n",
}}
