...
> 起因：1.35.0 全量基准跑完后，按 `dev_market_bench.py` 自动自检提醒逐条抽样，发现 `hardcoded_secret` 56 条 ERROR 占单轮全部 ERROR 的 50%，集中在变量赋值 / 占位符行。属第四处检查器缺陷，本轮治理。

### 修复

- **缺陷④ `SECRET_RE` 把标识符 / 函数返回值误判为硬编码密钥（security 检查器）**：旧正则 `...\s*[:=]\s*['\"]?[A-Za-z0-9_\-]{12,}` 中引号**可选**，于是任何 12+ 字符的标识符（变量名、函数名）都被当成密钥字面量。规模实测 `hardcoded_secret` 56 条中绝大多数为此类误报。修法：重构 `SECRET_RE` 为两分支严格口径——
  - 分支① 引号字面量：仅当值被引号包裹（如 `"AKIA..."` / `'sk-...'`）才判真硬编码；并排除**全大写占位符**（`YOUR_BOT_TOKEN_HERE` 类示例值）。
  - 分支② 无引号·行尾：仅当值到行尾且**含数字 / 大写 / 连字符**（即非 snake_case 变量名 / 占位符）才判，排除 `self.x = x`、`api_key=resolved_key`、`export X=your_api_key` 类赋值。
  - 关键字与分隔符之间允许一个可选引号，兼容 JSON `"api_key": "..."` 写法（旧正则本就不支持该写法，属既有漏报，本轮一并补齐）。
  - 关键坑：首版修复用全局 `(?i)` 导致 lookahead 里的 `[a-z_]+` 也大小写不敏感，反而把 `wJalrXUtnFEMI...`（真密钥）当「纯字母标识符」误拒；改为仅对关键字组 `(?i:...)` 局部不敏感、值部分保持大小写敏感后才正确。新正则是旧正则的**严格子集**（保留全部真阳性，消除全部误报），不引入新漏报。
  - 验证：同 50 技能直接全量扫描 `hardcoded_secret` **0 命中**（修复前 56）；`self_validate` dirty fixture `error=10` 未变（`API_KEY = "AKIA1234567890ABCDEFXYZ"` 真硬编码仍检出）；15 项正负例单元测试全 PASS（含 JSON / shell / 混合大小写真密钥，及变量赋值 / 函数返回值 / 占位符反例）。

### 说明

- 本次仅治理 security 检查器误报；1.35.0 的缺陷①②③与基准工具增强不受影响。
...
- **修复**：L185 `ast 零依赖 / skip 跳过` → `ast 零依赖 / off 不运行`。纯文档单点修正，无代码改动。四处版本号一致 1.29.5。

## 1.29.4 打磨明细（收口 DEVELOPMENT.md 命令表/指令清单的编号与计数措辞漂移，纯文档修正）

- **背景**：用户提出「同步钩子、本地 CI、远程 CI 的命令表 / 指令清单是否有更新或重新编撰的必要」。
  逐行核对 DEVELOPMENT.md 与真实代码后确认：**命令表与指令清单的结构完整、不需要重新编撰**，
  但存在 7 处编号 / 计数措辞漂移，均为 v1.27.21 指令清单重编号（10 类 → 8 类）与 v1.29.2
  新增 `bump_audit` 之后的收口遗漏。本次全部修正。
- **修正明细（7 处）**：
  1. **L124 DEV_TOOLS 计数过时**：「全 8 个」→「全 9 个」并补列 `bump_audit`（v1.29.2 新增后漏改）。
  2. **L22 post-commit 职责过时**：「post-commit 仅同步、不跑它（`dev_self_audit`）」→
     「post-commit 同步 + 版本 bump 提交经 `bump_audit` 自动跑它作早期反馈」（v1.29.2 后行为已变）。
  3. **L245 旧 10 类时代残留**：「第 5–9 类…与第 10 类（常驻）」→
     「第 5–8 类均来自 check-bump（#5 规模化基准 / #6 上架授权 / #7 文档叙述 / #8 常驻未提交）」——
     重编号后已无第 9 / 10 类。
  4. **L251 阻断归属错指**：「第 7–8 类进 `rel_block` 阻断（上架授权）」→
     「第 6 类上架授权进 `rel_block` 阻断、第 5/7/8 类进 `rel_info` 不阻断」——
     上架授权是 #6，#7/#8 均为 `[建议]` 不阻断。
  5. **L225 退役标题歧义**：「第 6 / 7 类已于 v1.27.21 退役」→「**旧**第 6 / 7 类…」，
     避免与现役 #6（上架授权）/ #7（文档叙述）混淆（正文原已用「旧第 6/7 类」区分）。
  6. **L292 远程 CI 提示计数窄**：「4 类提示」→「8 类提示（release_check 产 #1-4 + check-bump 产 #5-8）」。
  7. **L247 解释不完备**：原仅解释 #6（上架授权）为何全版本打印，未解释 #7；已补「#7 因版本变动叙述
     在任何级别迭代中都可能被误写入用户文档，故同样全版本提醒」。
- **验证**：7 处均用字节级替换（assert 命中 1 处）+ Read 复核真实磁盘；`dev_self_audit.py --strict`
  9/9 检查器 ERROR 0 / WARN 0（含 doc + doc-llm agent 模式，覆盖 README/CHANGELOG dev 文档）；
  `self_validate` 4/4 PASS。四处版本号一致 1.29.4。
