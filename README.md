...
| 1.27.19 | **pre-push 承接 [agent-todo] #6/#7（dev 工具增强）**：审计报告 tee 落盘 `bench/agent_audit_report.md`（钩子自清理），doc-llm 确定性「正向覆盖缺口」段触发拦 push 并给 dossier 路径（须 agent 接手判读），`SKILL_AUDIT_SKIP_DOC_LLM_GATE=1` 可放行有意缺口。不进部署副本。四处版本号一致 1.27.19 |
