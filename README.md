...
| 版本 | 说明 |
| --- | --- |
| 1.27.19 | **pre-push 承接 [agent-todo] #6/#7（dev 工具增强）**：审计报告 tee 落盘 `bench/agent_audit_report.md`（agent 读后删），doc-llm 确定性「正向覆盖缺口」段触发拦 push 并给 dossier 路径（须 agent 接手判读），`SKILL_AUDIT_SKIP_DOC_LLM_GATE=1` 可放行有意缺口。不进部署副本。四处版本号一致 1.27.19 |
| 1.27.18 | **dev_commit 回显 post-commit 钩子同步回执（dev 工具增强）**：修复 `capture_output` 吞掉钩子 `[sync_deploy] verify: OK/MISMATCH` 输出、成功分支仅打模糊措辞「应已同步」的缺陷——成功分支现原样回显钩子输出，agent/维护者无需再手动核验部署副本。dev-only，不影响发布面。四处版本号一致 1.27.18 |
| 1.27.17 | **全量文档↔代码交叉校对收口（纯文档修正）**：全量审计 + dossier 语义比对 + 人工核对版本/CLI 参数/错误码/检查器清单与 DEVELOPMENT.md。修复：① checkers.md 死代码节 vulture 描述与 SKILL.md/代码矛盾（漏「显式 vulture 缺库先自动 pip install」，已对齐并限定仅显式路径联网安装）；② checkers.md 参数速查表补全 9 个遗漏参数（`--doc-llm-mode`/`--examples-consent`/`--source`/`--ref`/`--keep-temp`/`--report`/`--target`/`--verify`/`--dev-docs`）；③ 措辞修正——examples「三档模式」实列 4 值改「多档模式」、跨平台证明 npm 实际不被调用改列 `pip`、「完整运行示例」改标注示意输出。机械层 3 ERROR/8 WARN 均溯源为 DEV_TOOLS 已知误报类，发布面用户文档零 ERROR。四处版本号一致 1.27.17 |
| 1.27.16 | **补记 v1.27.0 doc-llm 正向覆盖缺口能力的用户文档同步（纯文档修正）**：v1.27.0 落地正向能力覆盖时 CHANGELOG 已记录「doc-llm dossier 新增『正向覆盖缺口』预分析段（共用 `compute_capability_gaps()`）」，但 SKILL.md 与 references/checkers.md 的 doc-llm 描述均未体现。本版在 SKILL.md 检查器清单与 checkers.md 总览补写该能力（确定性列出代码已注册但文档未写的检查器 / CLI 参数，列为 agent 比对要点优先核对）与「代码事实清单」构成；checkers.md `doc_llm_agent_handoff` 行补 dossier 内容。无代码改动。四处版本号一致 1.27.16 |
| 1.27.15 | **doc-llm 事实清单退出码口径与 doc 检查器对齐（修复并行升级遗漏）**：v1.27.12 重写 `doc.py` 退出码比对（`sys.exit` 真实进程退出码）时，漏改 `doc_llm.py` 的 `_code_fact_sheet`——其仍用旧的 `return\s+(\d+)` 提取「返回码」，会误把函数 `return 0/2` 当退出码，与 doc 口径不一致且会误导语义比对。本版在 `core.py` 抽共享 helper `extract_code_exit_codes(blob)`（只认 `sys.exit(N)`、覆盖条件表达式两种分支），doc 与 doc-llm 共用同一口径，杜绝两套实现漂移；事实清单键名「返回码」同步更正为「退出码」。`DEV_TOOLS` 单一真相源移至 `core.py`，`doc.py` A3 退出码比对按文件排除 dev 工具（dev 专用码如 `sys.exit(42)` 不再误报 `EXIT_CODE_ONLY`），直接 CLI 审计 src 与发布门禁行为对齐。`doc.py` A3 与 `doc_llm.py` dossier 比对要点「退出码」维度现已一致。四处版本号一致 1.27.15 |
| 1.27.14 | **补全 v1.27.13「去散文」收口（纯文档修正）**：v1.27.13 已用代码 consent 闸门（`--examples-consent`）接管 examples 弹窗强约束并删除「Agent 执行约定」整段，但 SKILL.md 仍残留 3 处指向该已删章节的悬空引用（deadcode 精度说明、dev 模式自动降级提示、examples 弹窗约定 bullet）。本版以子串替换彻底清除全部「Agent 执行约定」字样，文档仅留用法参考、强制逻辑完全代码化，兑现「少靠散文、省篇幅」意图。无功能改动、不破坏 self_validate 黄金快照。四处版本号一致 1.27.14 |
| 1.27.13 | **examples 弹窗强约束改由代码 consent 闸门自执行（去散文）**：`examples.py` 新增 `--examples-consent` 授权令牌——非交互（agent）环境显式指定 `--examples-mode run/static/off` 须携带该令牌，否则阻断执行并报告 `examples_consent_missing`（ERROR），杜绝 agent 静默替用户决定档位；默认 ask 在非交互下降级为纯静态并发出 `user_prompts`（JSON 顶层 + 报告「⚠ 需用户决策」块），由 agent 转交用户决策。删除 SKILL.md「Agent 执行约定」整段强制散文（deadcode/doc-llm/examples/全局铁律），仅保留用法参考——强制逻辑完全代码化，兑现「少靠散文、省篇幅」意图。`dev_self_audit` 审计自家源码携带 `examples_consent=True` 授权 run 模式。四处版本号一致 1.27.13 |
| 1.27.12 | **修复 EXIT_CODE_ONLY 误报 + 硬化 examples 弹窗强约束**：`doc.py` 退出码比对正则重写——`DOC_EXIT_RE` 兼容「表格行」与「行内反引号（`退出码：\`0\`...\`130\``）」两种写法，`CODE_EXIT_RE` 改匹配真实进程退出 `sys.exit(<arg>)` 并从实参提取数字（覆盖 `sys.exit(0 if failed else 1)` 条件分支），不再误匹配函数 `return N`（此前误抓 DEV_TOOLS 的 `return 0/2` 致 `EXIT_CODE_ONLY 0/2` 误报）。`SKILL.md`「Agent 执行约定」补齐 examples 弹窗红线 + 新增「user_prompts 必须弹窗」全局铁律（agent 不得单方面传 `--examples-mode` 规避询问）；`report.py` 人类报告末尾新增醒目的「⚠ 需用户决策」块（与 JSON 顶层 `user_prompts` 双通道送达，杜绝读漏未弹窗）。四处版本号一致 1.27.12 |
