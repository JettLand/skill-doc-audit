...
| 1.8.0 | 4.8/5 优秀 | **deadcode 投产打磨**：跨文件引用感知 + 多语言扫描覆盖（.ts/.go/.rs 等）；新增 `--preview` 检查预览；`UNKNOWN_IDENT` 误报修复（ERROR→WARN，按标识符去重）；文档渐进式披露（快速开始/速查表） |
| 1.7.0 | 无评测记录（平台仅保留最新版报告，历史版本报告不可查） | deadcode 并入 --all-checks 默认集；运行前按 --deadcode-mode 询问精度（vulture/ast/skip），超时回退 ast |
